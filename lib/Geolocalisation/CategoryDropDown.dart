...
class CategoryDropdown extends StatefulWidget {
  final Function(String) onCategorySelected; // Ajout du callback pour la catégorie sélectionnée

  CategoryDropdown({required this.onCategorySelected}); // Modifiez le constructeur pour accepter le callback
...
class _CategoryDropdownState extends State<CategoryDropdown> {
  ...
  Future<void> fetchCategories() async {
    final response = await http.get(Uri.parse('http://localhost:8080/api/categories/getCategories'));
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      padding: EdgeInsets.symmetric(horizontal: 7.0), // Adjust padding for compactness
      ...
      child: SizedBox(
        ...
        child: DropdownButtonHideUnderline( // Remove default underline
          child: DropdownButton<String>(
            ...
            hint: Text(
              'Select a category',
              style: TextStyle(fontSize: 14), // Adjust font size for compact appearance
            ),
            icon: Icon(Icons.arrow_drop_down, size: 24), // Adjust the icon size
            onChanged: (String? newValue) {
              setState(() {
                selectedCategory = newValue;
                widget.onCategorySelected(newValue!); // Appel du callback avec la catégorie sélectionnée
                ...
                child: Row(
                  children: [
                    if (category['iconPath'] != null)
                      Image.asset(
                        category['iconPath'], // Load icon from asset path if available
